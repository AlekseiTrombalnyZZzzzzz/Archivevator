"""Основная логика: стратегия, мульти-пасс, верификация, сжатие/распаковка, гибридный Office."""
...
def compress_file(
    ...
    optimize_office: bool = False, lossy_office: bool = False
) -> Dict[str, object]:
    ...
    if src.suffix.lower() in ('.docx', '.xlsx', '.pptx') and optimize_office:
        ...
        return opt_off(src, dst, lossy=lossy_office, verify=verify, delete_source=False)
    ...
    try:
        ...
        if result["original_size"] < config.SMALL_FILE_THRESHOLD:
            logger.info(f"⏭️ Пропуск {src.name}: слишком мал (<{config.SMALL_FILE_THRESHOLD}B)")
        ...
        if entropy > 0.92 and not force and not is_image: logger.warning("⚠️ Высокая энтропия. Сжатие ограничено.")
        ...
        result.update(algorithm=best_algo, compressed_size=best_size, ratio=best_size/result["original_size"])
        ...
        except Exception as e: logger.warning(f"⚠️ Метаданные частично: {e}")
...
def compress_hybrid_office(
    ...
    """Гибридный режим: Office Opt -> Entropy Compression. Даёт максимальную экономию."""
    src = Path(src).resolve()
...
def decompress_file(
    ...
    try:
        ...
        if verify:
            ...
            if meta_path.exists():
                ...
                if final_algo == "image":
                    ...
                    logger.info("🛡️ Структура изображения валидна")
                elif expected:
        ...
        # Всегда удаляем исходный файл после успешной распаковки
        if result["status"] == "ok":
            try:
                src.unlink()
                meta_path = src.with_suffix(src.suffix + ".meta.json")
                if meta_path.exists():
                    meta_path.unlink()
                logger.info(f"🗑️ Исходный файл удалён: {src.name}")
            except Exception as e:
                logger.warning(f"⚠️ Не удалось удалить исходник: {e}")
